...
function wind10to2(u10) {
  // FAO-56: u2 = u_z * 4.87 / ln(67.8 z - 5.42), z=10
...
function gamma_kpa_per_c(P_kpa) {
  return 0.000665 * P_kpa;
}

function wpm2_to_mjpm2h(w) {
  return w * 3600.0 / 1_000_000.0;
}

// Solar geometry (FAO-56 style)
...
function solarDeclination(J) {
  return 0.409 * Math.sin(2*Math.PI*J/365 - 1.39);
}

function invRelDistES(J) {
...
function hourAngle(solarTimeHours) {
  return Math.PI/12.0 * (solarTimeHours - 12.0);
}
...
function extraterrestrialRaHourly(localISO, latDeg, lonDeg, tzHours) {
  // localISO like "2025-12-13T12:00"
...
function computeEt0HourlyPM({localISO, lat, lon, alt, tzHours, T, RH, u10, P_hPa, Rs_W}) {
  ...
  // Soil heat flux: day ~0.1Rn, night ~0.5Rn (use Rs>0 as daytime proxy)
  const G = (Rs > 0) ? 0.1*Rn : 0.5*Rn;

  const num = 0.408*Delta*(Rn - G) + gamma*(37.0/(Tc + 273.0))*u2*vpd;
  const den = Delta + gamma*(1.0 + 0.34*u2);

  const et0 = (den !== 0) ? Math.max(0.0, num/den) : 0.0; // mm/h
  return { et0, u2, Rn, Ra, Rso };
}

// ---------- State to support report/PDF ----------
const STATE = {
  api: null,    // {inputs, hourlyRows, daily, summary}
  manual: null  // {mode, inputs, hourlyRows, summary}
};

// ---------- API run ----------
async function runApi() {
  try {
    setStatus("API yuklanmoqda...");
    ...
    // time-based decision only if time provided
    let timeDecision = "";
    if (t) {
      const target = `${d}T${t}`;
    ...
    STATE.api = {
      ...
      hourlyRows: times.slice(0, 24).map((tm, i) => ({ time: tm, soil: soilPct[i], et0: et0[i], etc: etc[i] })),
      hourlyRows12: times.slice(0, 12).map((tm, i) => ({ time: tm, soil: soilPct[i], et0: et0[i], etc: etc[i] })),
      totals: { et0Day, etcDay },
      raw: { firstRawIdx, rawMsg },
      timeDecision: t ? { target: `${d}T${t}`, shown: true } : { shown: false }
    };
...
function make24Hours(dateStr) {
  ...
  for (let h=0; h<24; h++) {
    ...
    seriesAddRow({ time: `${dateStr}T${hh}:00`, T:"", RH:"", u10:"", P:"", Rs:"", cloud:"", soil:"" });
  }
}
...
function manualCommonInputs() {
  ...
  if (!t) throw new Error("Manual: vaqt kiritilmagan (manual rejimda vaqt kerak).");

  return { lat, lon, alt, d, t, kc, raw, soil };
}

function getTzHoursFromISO(isoLocal) {
  // Open-Meteo provides local ISO without timezone; for manual we approximate tz from browser.
  // This is acceptable for a web tool; precision impact is small for ET0.
...
function runManualSnapshot() {
  try {
    ...
    const base = manualCommonInputs();
    const timeISO = `${base.d}T${base.t}`;
    const tzHours = getTzHoursFromISO(timeISO);
    ...
    // cloud provided but not directly used (we use Rs/Rso proxy); keep for completeness
    const _cloud = parseFloat($("s_cloud").value);

    const out = computeEt0HourlyPM({
      localISO: timeISO,
      lat: base.lat, lon: base.lon, alt: base.alt,
      tzHours,
    ...
    STATE.manual = {
      ...
      hourlyRows: rows,
      hourlyRows12: rows,
      totals: { et0Day: et0, etcDay: etc }, // snapshot treated as “hourly value”, report will label accordingly
      decisionAtTime: { time: timeISO, should, soil: base.soil, raw: base.raw }
    };
...
function runManualSeries() {
  try {
    ...
    const base = manualCommonInputs();
    const tzHours = getTzHoursFromISO(`${base.d}T${base.t}`);
    ...
    const calcRows = rowsIn.map(r => {
      ...
      if (![T,RH,u10,P,Rs].every(Number.isFinite)) {
        throw new Error(`Series: qiymatlar yetishmayapti yoki noto‘g‘ri (time=${time}).`);
      }
      ...
      // soil: per-row optional, else base.soil
    ...
    // decision at user-selected time (closest next row)
    ...
    const summaryHtml = `
      <div><b>Manual (hourly series)</b></div>
      <div><b>Joylashuv:</b> lat=${base.lat.toFixed(6)}, lon=${base.lon.toFixed(6)}, alt=${base.alt.toFixed(1)} m</div>
      <div><b>Sana:</b> ${base.d} | <b>Tekshiruv vaqti:</b> ${calcRows[idx].time}</div>
      <div><b>Kc:</b> ${base.kc.toFixed(2)} | <b>RAW:</b> ${base.raw.toFixed(1)}% | <b>Soil (tekshiruv):</b> ${soilAt.toFixed(1)}%</div>
      <hr style="border:0;border-top:1px solid #243047;margin:10px 0;">
    ...
    STATE.manual = {
      mode: "series",
      inputs: base,
      hourlyRows: calcRows.slice(0, 24),
      hourlyRows12: calcRows.slice(0, 12),
      totals: { et0Day, etcDay },
      decisionAtTime: { time: calcRows[idx].time, should, soil: soilAt, raw: base.raw }
    };
...
function buildReportPreview() {
  ...
  if (api) {
    lines.push(`<div><b>API:</b> ΣET0=${api.totals.et0Day.toFixed(2)} mm, ΣETc=${api.totals.etcDay.toFixed(2)} mm</div>`);
  } else lines.push(`<div><b>API:</b> mavjud emas</div>`);

  if (man) {
    lines.push(`<div><b>Manual (${man.mode}):</b> ΣET0=${man.totals.et0Day.toFixed(2)} mm, ΣETc=${man.totals.etcDay.toFixed(2)} mm</div>`);
  } else lines.push(`<div><b>Manual:</b> mavjud emas</div>`);
...
function fillPdfReportDom() {
  ...
  $("reportMeta").innerHTML = `
    ...
    <div><b>Source:</b> Browser + Open-Meteo (API), Manual (FAO-56)</div>
  `;
  ...
  const apiBlock = api
    ? `
      ...
      <div style="margin-top:8px;">${api.raw.rawMsg.replaceAll("<b>","").replaceAll("</b>","")}</div>
    `
  ...
  const manBlock = man
    ? `
      ...
      <div style="margin-top:8px;"><b>Tekshiruv qarori:</b> ${man.decisionAtTime.should ? "SUG‘ORISH KERAK" : "SUG‘ORISH SHART EMAS"} (${man.decisionAtTime.time})</div>
    `
    : `<div>Manual natijalari mavjud emas.</div>`;

  $("reportApiBlock").innerHTML = apiBlock;
  $("reportManualBlock").innerHTML = manBlock;

  let compare = `<div>Taqqoslash uchun API va Manual hisoblashlari kerak.</div>`;
  if (api && man) {
    const dEt0 = man.totals.et0Day - api.totals.et0Day;
    const dEtc = man.totals.etcDay - api.totals.etcDay;
    compare = `
      ...
      <div style="margin-top:8px;"><b>Qaror taqqoslash:</b></div>
      <div>Manual: ${man.decisionAtTime.should ? "Sug‘orish kerak" : "Sug‘orish shart emas"} (${man.decisionAtTime.time})</div>
    `;
    if (api.inputs.t) compare += `<div>API: vaqt kiritilgan (qaror API summary’da ko‘rsatiladi)</div>`;
    else compare += `<div>API: vaqt kiritilmagan (qaror ko‘rsatilmaydi)</div>`;
  }
  $("reportCompareBlock").innerHTML = compare;

  // Fill report tables (first 12 hours)
  const apiTb = $("reportApiTable").querySelector("tbody");
  apiTb.innerHTML = "";
  if (api) {
    api.hourlyRows12.forEach(r => addRow(apiTb, [r.time, r.soil.toFixed(1), r.et0.toFixed(3), r.etc.toFixed(3)]));
  }

  const manTb = $("reportManualTable").querySelector("tbody");
  manTb.innerHTML = "";
  if (man) {
    man.hourlyRows12.forEach(r => addRow(manTb, [r.time, r.soil.toFixed(1), r.et0.toFixed(3), r.etc.toFixed(3)]));
  }
}

async function downloadPdf() {
  // Build report DOM before export
  fillPdfReportDom();

  const node = $("reportPdf");
  node.classList.remove("hidden"); // needed for render
...
$("buildReport").addEventListener("click", () => {
  buildReportPreview();
  // also refresh the PDF DOM for preview consistency
  fillPdfReportDom();
  setStatus("");
});
...
// Initialize defaults
initDateDefault("api_date");
initDateDefault("m_date");
